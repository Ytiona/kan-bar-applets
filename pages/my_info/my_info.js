// pages/my_info/my_info.js
Page({

  /**
   * 页面的初始数据
   */
  data: {
    nameModalVisible: false,
    declarationModalVisible: false,
    sexModalVisible: false,
    regionModalVisible: false,
    QQModalVisible: false,
    region: [],
    currentCitys: [],
    selectedRegionIdx: [0,0]
  },

  /**
   * 生命周期函数--监听页面加载
   */
  onLoad: function (options) {
    const timer = setInterval(() => {
      const region = getApp().globalData.region;
      if(region){
        this.setData({ region, currentCitys: region[0].districts });
        clearInterval(timer);
      }
    }, 200)
  },

  showEditModal(e) {
    this.setData({ [e.currentTarget.dataset.param + 'ModalVisible']: true })
  },

  regionPickerChange(e) {
    if(this.data.selectedRegionIdx[0] !== e.detail.value[0]) {
      this.setData({ 
        currentCitys: this.data.region[e.detail.value[0]].districts,
        selectedRegionIdx: [e.detail.value[0], 0]
      })
    }
  },
  
  /**
   * 生命周期函数--监听页面初次渲染完成
   */
  onReady: function () {

  },

  /**
   * 生命周期函数--监听页面显示
   */
  onShow: function () {

  },

  /**
   * 生命周期函数--监听页面隐藏
   */
  onHide: function () {

  },

  /**
   * 生命周期函数--监听页面卸载
   */
  onUnload: function () {

  },

  /**
   * 页面相关事件处理函数--监听用户下拉动作
   */
  onPullDownRefresh: function () {

  },

  /**
   * 页面上拉触底事件的处理函数
   */
  onReachBottom: function () {

  },

  /**
   * 用户点击右上角分享
   */
  onShareAppMessage: function () {

  }
})